SELECT gcf.project_id, gcf."name", gcf.network, gcf.self_link AS link, gcf.direction, gcf.source_ranges, gcfa.ip_protocol, gcfa.ports, pr.range_start, pr.range_end, pr.single_port
...
FROM ( SELECT cq_id, Split_part(p, '-', 1) :: integer AS range_start, split_part(p, '-', 2) :: integer AS range_end, NULL  AS single_port
