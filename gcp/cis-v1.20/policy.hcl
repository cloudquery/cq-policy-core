policy "cis-v1.30" {
  ...
  view "project_policy_members" {
    description = "AWS Log Metric Filter and Alarm"
    query "project_policy_members_query" {
  ...
  policy "gcp-cis-section-2" {
    ...
    view "log_metric_filters" {
      description = "GCP Log Metric Filter and Alarm"
      query "log_metric_filters_query" {
    ...
    query "2.4" {
      ...
      expect_output = true
      query = <<EOF
        SELECT * FROM log_metric_filters WHERE
        enabled = TRUE
  ...
  policy "gcp-cis-section-5" {
    ...
    view "public_buckets_check" {
      description = "Aggregated buckets and their access params"
      query "public_buckets_check_query" {
    ...
    query "5.1" {
      ...
      query = <<EOF
        SELECT project_id , "name", self_link as link from public_buckets_check
        WHERE member LIKE '%allUsers%'
        OR member LIKE '%allAuthenticatedUsers%'
